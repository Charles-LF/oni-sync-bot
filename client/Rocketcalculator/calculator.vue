...
const MOBILE_BREAKPOINT = 768; // 和CSS媒体查询阈值完全统一
...
onUnmounted(() => {
    ...
    if (resizeTimer) clearTimeout(resizeTimer); // 清理定时器
});
...
const handleCalculate = () => {
    try {
        ...
        if (res.length > 0) {
            ...
            // 修复移动端滚动逻辑，适配el-scrollbar自定义容器
            nextTick(() => {
                if (isMobile.value && scrollBarRef.value && resultCardRef.value) {
